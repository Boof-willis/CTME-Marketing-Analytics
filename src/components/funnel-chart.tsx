...
export interface FunnelStage {
  ...
  /**
   * Optional value used only for the segment geometry (thickness). Lets callers
   * keep tiny stages visible while the percentage badge still reflects the real
   * `value`. Falls back to `value` when omitted.
   */
...
export function FunnelChart({
  ...
  const n = data.length;
  // Percentage base is the largest stage, not blindly the first one. If the
  // first stage is 0 (e.g. 0 new leads today but an appointment from an older
  // contact), dividing by it would yield Infinity%/NaN%.
  const max = Math.max(...data.map((d) => d.value), 0);
  // Geometry uses geometryValue (when provided) so small stages stay visible;
  // normalize against the largest geometry value and clamp to [0,1] so no
  // segment can balloon past the funnel when the data isn't monotonic.
  const geomVals = data.map((d) => d.geometryValue ?? d.value);
  const geomMax = Math.max(...geomVals, 0) || 1;
  const norms = geomVals.map((v) => Math.min(1, Math.max(0, v / geomMax)));
